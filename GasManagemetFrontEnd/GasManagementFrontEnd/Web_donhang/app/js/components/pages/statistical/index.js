...
function Statistical() {
    ...
    if (result.data.data.length !== 0) {
      result.data.data.map((value) => {
        ...
        value.detail.map((v) => {
          ...
          sumNumberSale += v.statistic.numberSale;
          sumTurnback += v.statistic.turnback;
          ...
          sumChildNumberSale += v.statistic.numberSale;
          ...
          //THỐNG KÊ Ở ĐẠI LÝ, KHÁCH HÀNG CÔNG NGHIỆP,...
          else if ((userType === "Agency" || userType === "General")) {
            // Tạo object tính tổng
            sumObject = {
              name: "Tổng",
              cancel: sumCancel,
            ...
            array.push({
              ...
              turnback: v.statistic.turnback,
              returnFullCylinder: v.statistic.returnFullCylinder,
              numberSale: v.statistic.numberSale
            });
          }
  ...
  async function exportExcelDashboard(target_ids, action_type, parent_root, start_date, end_date, name = '', type = '') {
    let result = await exportExcel(target_ids, action_type, parent_root, start_date, end_date, name, type);
    setLoading(false);
  }
  async function handleExportExcel() {
    ...
    if (typeExcel === "") {
      ...
      if (userRole === "Owner" && userType === "Factory") {
        setLoading(true);
        exportExcelDashboard([user_cookies.user.id], typeExcel, user_cookies.user.parentRoot, startTime, endTime, nameCustom, typeList[typeExcel]);
        ...
        if (idStation) {
          ...
          if (idStation === "all") {
            ...
            exportExcelDashboard([idStation], typeExcel, user_cookies.user.parentRoot, startTime, endTime, nameCustom, typeList[typeExcel]);
          }
        } else {
          showToast("Vui lòng chọn trạm");
        }
      } else if (userRole === "SuperAdmin" && (userType === "Agency" || userType === "General")) {
        setLoading(true);
        exportExcelDashboard([user_cookies.user.id], typeExcel, user_cookies.user.parentRoot, startTime, endTime, nameCustom);
      }
      else if (!nameBranch) {
        showToast("Vui lòng chọn Chi Nhánh");
      } else {
        if (userRole === "SuperAdmin" && userType === "Factory") {
          ...
          if (idBranch === "all") {
            ...
            exportExcelDashboard([idBranch], typeExcel, user_cookies.user.parentRoot, startTime, endTime, nameCustom, typeList[typeExcel]);
          } else if (idBranch === null && idStation) {
            exportExcelDashboard([idStation], typeExcel, user_cookies.user.parentRoot, startTime, endTime, nameCustom, typeList[typeExcel]);
          }
        }
  ...
  const typeList = type_excel_station.reduce((acc, curr) => (acc[curr.key] = curr.name, acc), {}); //shorthand gan object va return lai object
    ...
    <div className="section-statistical" id="statistical">
      ...
      <div className="section-statistical__report">
                  ...
                  <Select placeholder="Chọn" style={{ width: 120 }} onChange={handleChangeExcel}>
                    {(userRole === "SuperAdmin" && userType === "Fixer") ||
                      ...
                      (userType === "Agency" || userType === "General") ?
                        type_excel_station_agency.map((value, index) => {
                          return (
                            <Option key={index} value={value.key}>
                              {value.name}
                            </Option>
        ...
        <div className="section-statistical__report__body">
          ...
          ) : (userRole === "SuperAdmin" && (userType === "Agency") || userType === "General") ?
            <div className="section-statistical__report__body">
              <div className="container">
                <Table columns={columnsForAgency} dataSource={listStationDashboard} scroll={{ x: 1200 }} bordered />
              </div>
            </div> :
            ""
          }
