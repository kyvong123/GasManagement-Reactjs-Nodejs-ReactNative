    ...
    bcrypt
			...
			.then(isValid => {
			  if (!isValid) {
			    ...
			    SystemUsermanager._generateUserToken(user, token => {
...
module.exports = {
  ...
  createUser: async (values) => {
    ...
    try {
      const user = await SystemUser.create(values).fetch();
      if (!user) {return {success: false, err: API_ERRORS.SERVER_ERROR};}

      const token = SystemUsermanager._generateUserToken(user);
				    ...
				    User
							...
							.then(async (patchedUser) => {
							 ...
							 let updatedUser = await User.updateOne({id: patchedUser.id}).set({encryptedPassword: patchedUser.encryptedPassword});
							  ...
							  //     resolve(token);
							  //   });
							  const token = SystemUsermanager._generateUserToken(updatedUser);
  ...
  updateUser: function (user, newPassword, updateInfo = {}) {
    return new Promise((resolve, reject) => {
      if(newPassword) {
        SystemUser.setPassword(user, newPassword).then(async (patchedUser) => {
          patchedUser.resetToken = null;
          patchedUser.passwordFailures = 0;
		  patchedUser.lastPasswordFailure = null;
        ...
        SystemUser.updateOne({id: user.id}).set(updateInfo).then(
			updatedUser => {
