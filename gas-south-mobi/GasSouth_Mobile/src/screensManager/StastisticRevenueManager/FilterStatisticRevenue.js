...
import { getListOrderByDate, getRevenue } from '../../api/orders_new';
import { useSelector, useDispatch } from 'react-redux';
// import { listOrderofOrderManager } from '../../actions/listOrderActions';

const FilterStatisticRevenue = (props) => {
    ...
    const geOrderByDate = async () => {
        try {
            const data = await getRevenue(date2.firstDay, date2.lastDay)
            if (data.success) {
                // dispatch(listOrderofOrderManager(data.data))
                props.navigation.navigate('statisticRevenue', { date, list: data.data })
            } else {
                // dispatch(listOrderofOrderManager([]))
                props.navigation.navigate('statisticRevenue', { date, list: [] })
            }
        } catch (err) {
            console.log("LỖI FETCH API,", err);
        }
        props.navigation.navigate('statisticRevenue', { date })
    }
...
export default FilterStatisticRevenue;