...
export default function StatisticDetailsVer2(props) {
  ...
  const cylinderTypes = ['B6', 'B12', 'B20', 'B45'];
  ...
  const calTotalX = (data, dataColumn) => {
    ...
    data.map(child => {
      dataColumn.map(statistic => {
        ...
        if (statistic == 'numberExport') {
          dictTmp[statistic]['TotalKg'] += totalYDict[child.name][statistic + 'kg']
        }
      })
    })
    return dictTmp;
        ...
        child.detail.map(value => {
          ...
          if (statistic == 'numberExport') {
            if (child.name === 'Bình Khí') {
              ...
              dictTmp[child.name][statistic + 'kg'] += value.statistic[statistic] * parseInt(value.name.split(' ')[1])
            }
          }
        })
      })
      ...
      if ((userInfo.userRole == 'SuperAdmin' && userInfo.userType == 'Factory')
        ...
        await statisticsApi.getFixerDetail(
          ...
          dataColumnFixer.map(statistic => {
            let dictTmp1 = {};
            cylinderTypes.map(type => {
              dictTmp1[type] = data.data.data[0].detail.filter(value => {
                return type.includes(value.categoryCode)
              })
            })
          ...
          dataColumnFixer.map(statistic => {
            cylinderTypes.map(type => {
              data.data.data[0].detail.map(detail => {
                if (type.includes(detail.categoryCode)) {
                      ...
                      <View style={styles.tableStyle}>
                        ...
                        <View>
                          {cylinderTypes.map(type => {
                            return (
                              <View style={styles.rowStyle}>
                                <View style={[styles.elementStyle, { width: widthScreen / 3, height: displayDataFixer[statistic][type].length * 25 }]}>
                                  <Text style={styles.elementText}>{type}</Text>
                                </View>
                                <View style={{ flexDirection: 'column' }}>
                                  {displayDataFixer[statistic][type].map(value => {
                                    return (
