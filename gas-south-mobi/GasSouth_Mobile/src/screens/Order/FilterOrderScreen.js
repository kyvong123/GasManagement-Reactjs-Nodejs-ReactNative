...
import { getListOrderByDate } from '../../api/orders_new';
import { useSelector, useDispatch } from 'react-redux';
import { listOrderofOrderManager } from '../../actions/listOrderActions';

const FilterOrderScreen = (props) => {
    ...
    const geOrderByDate = async () => {
        try {
            const data = await getListOrderByDate(date2.firstDay, date2.lastDay)
            console.log(data)
            // data.success ? dispatch(listOrderofOrderManager(data.data)) :

            if (data.success) {
                // dispatch(listOrderofOrderManager(data.data))
                props.navigation.navigate('OrderManagement', { date, list: data.data })
            } else {
                // dispatch(listOrderofOrderManager([]))
                props.navigation.navigate('OrderManagement', { date, list: [] })
            }
        } catch (err) {
