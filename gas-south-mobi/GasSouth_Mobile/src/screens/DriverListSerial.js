...
import { storeSerialAction, deleteSerialAction } from '../actions/RefluxAction';
import { SHIPPING_TYPE } from '../constants';

const DriverListSerial = () => {
    ...
    const handleUserPressBtnContinues = () => {
        switch (currentTypeReflux) {
            ...
            case SHIPPING_TYPE.HOI_LUU_KHO_XE:
                Actions.push("VehicleInventory_FormSubmit");
                break;
            default:
                alert("LỖI chuyển màn hình");
                break;
        }
    }
    return (
        <SafeAreaView>
            ...
            <View style={tw('flex items-center justify-between flex-row bg-[#008a00] py-4 px-3')}>
                ...
                <Text style={tw('text-lg text-white font-black ')}>{currentTypeReflux === SHIPPING_TYPE.TRA_BINH_DAY ? "serial bình đầy" : "serial vỏ bình"}</Text>
                <View>
            ...
            <View style={{ height: '100%' }}>
                {listSerialInStore.length === 0 ?
                    ...
                    <ScrollView >
                        {listSerialInStore.map(itemSerial =>
                            <View style={[tw('flex-row justify-between my-1 bg-gray-300 items-center'), { borderRadius: 5, paddingHorizontal: 30, marginVertical: 10, paddingVertical: 1, marginHorizontal: 10 }]}>
                                <View style={tw('w-3/4')}>
                                    <Text style={tw('text-lg font-black')}>{itemSerial.serial}</Text>
                                </View>
                                <View style={tw('')}>
                                    <DeleteIcon onPress={() => { dispatch(deleteSerialAction(itemSerial.serial)) }} color={'#fc1703'} size={25} name="delete" />
                                </View>
                            </View>
                        )}
                    </ScrollView>
                ...
                <View style={[tw('flex-row justify-between'), { height: '25%', marginHorizontal: 20, marginTop: 20 }]}>
                    <Text style={{ fontWeight: 'bold', fontSize: 20, marginLeft: 20, paddingTop: 10 }}>{`Tổng: ${listSerialInStore.length} ${currentTypeReflux === SHIPPING_TYPE.TRA_BINH_DAY ? 'bình đầy' : 'vỏ bình'}`}</Text>
                    <Button
                        isDisabled={listSerialInStore.length === 0 ? true : false}
                        onPress={handleUserPressBtnContinues}
                        style={[tw(''), { height: 50 }]}
                        colorScheme="orange">
                        Tiếp tục
                    </Button>
...
export default DriverListSerial
