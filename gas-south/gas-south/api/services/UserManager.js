	...
	authenticateUserByPassword: function (email, password) {
		return new Promise((resolve, reject) => {
			UserManager
				.validatePassword(email, password)
				.then(({ isValid, user }) => {
					// Hàm chuyển đổi chuỗi thành mảng ký tự ASCII
					// function stringToASCIIArray(str) {
					// 	return str.split('').map(char => char.charCodeAt(0));
					// }
					// // Hàm mã hóa Base64 không có Padding
					...
					// 		.replace(/=+$/, '');
					// }
					// const code_verify = sails.config.custom.code_verify
					// // Bước 3: Hash chuỗi ASCII bằng SHA-256
					// const sha256Hash = CryptoJS.SHA256(code_verify).toString(CryptoJS.enc.Hex);

					// // Bước 4: Mã hóa Base64 không có Padding
					// const base64Encoded = base64_urlencode(sha256Hash);

					// console.log('code_challenge', base64Encoded)
