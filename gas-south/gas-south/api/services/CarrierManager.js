...
async function verifyUser(email, password) {
  try {
    // return !!await Carrier.findOne({email});

    const user = await Carrier.findOne({email: email});
    ...
    bcrypt
			...
			.then(isValid => {
			  if (!isValid) {
			    ...
			    CarrierManager._generateUserToken(user, token => {
...
module.exports = {
  ...
  createUser: async (values) => {
    ...
    try {
      const user = await Carrier.create(values).fetch();
      if (!user) {return {success: false, err: API_ERRORS.SERVER_ERROR};}

      const token = CarrierManager._generateUserToken(user);
				  ...
				  if (!isValid) {
				    ...
				    Carrier
							...
							.then(async (patchedUser) => {
							 ...
							 let updatedUser = await Carrier.updateOne({id: patchedUser.id}).set({encryptedPassword: patchedUser.encryptedPassword});

							  //   CarrierManager._generateUserToken(updatedUser, token => {
							  //     resolve(token);
							  //   });
							  const token = CarrierManager._generateUserToken(updatedUser);
  ...
  updateUser: function (user, newPassword, updateInfo = {}) {
    return new Promise((resolve, reject) => {
      if(newPassword) {
        Carrier.setPassword(user, newPassword).then(async (patchedUser) => {
          patchedUser.resetToken = null;
          patchedUser.passwordFailures = 0;
		  patchedUser.lastPasswordFailure = null;
          ...
          let updatedUser = await Carrier.updateOne({id: patchedUser.id}).set(updatedata);
          const token = CarrierManager._generateUserToken(updatedUser);
          return resolve({updatedUser, token});
        }).catch(reject);
      } else {
        Carrier.updateOne({id: user.id}).set(updateInfo).then(
			updatedUser => {
  ...
  resetPasswordByResetToken: function (email, resetToken, newPassword) {
    return new Promise((resolve, reject) => {
      Carrier
				...
				.exec((err, carrier) => {
				  if (err) {return reject(err);} // Query error
				  if (!carrier) {return reject(API_ERRORS.USER_NOT_FOUND);}
				  ...
				  carrier
						...
						.then(() => {
							carrier.resetToken = null;
							carrier.passwordFailures = 0;
							carrier.lastPasswordFailure = null;
							carrier.save();
