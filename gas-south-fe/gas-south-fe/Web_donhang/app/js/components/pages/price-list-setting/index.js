...
export default class PriceSetting extends Component {
  ...
  async getListPrice(id, token) {
    let GETLISTPRICE = SERVERAPI + "8/price/getAllPrice";
  ...
  getListCylinder(id, token) {
    return axios({
      method: "GET",
      url: SERVERAPI + `/categoryCylinder/list?id=${id}`,
      headers: {
        Authorization: token,
      },
    })
  ...
  async addCylinder(cylind) {
    ...
    let data = cylind;
    let ADDCYLINDER = SERVERAPI + "/price/create";
  ...
  async deletePrice(id) {
    ...
    let DELETEPRICE = SERVERAPI + "/price/cancelPrice";
  ...
  async updatePriceAPI(data) {
    let user_cookies = await getUserCookies();
    let token = "Bearer " + user_cookies.token;
    let UPDATEPRICE = SERVERAPI + "/price/updatePrice";
