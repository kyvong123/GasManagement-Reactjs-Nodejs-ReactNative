...
class AddUserPopupContainer extends React.Component {
    ...
    if (user) {
      if (user.status === Constants.HTTP_SUCCESS_CREATED) {
        ...
        showToast(
          ...
          showToast("Email này đã tạo tài khoản rồi. Mời bạn nhập lại!", 2000)
        );
  ...
  async updateUser(
    ...
    //console.log('register',user);
    if (user) {
      if (user.status === Constants.HTTP_SUCCESS_BODY && !user.data.err_msg) {
  ...
  render() {
    return (
      <AddUserPopup
        ...
        isViewForm={this.props.isViewForm}
        updateUser={this.updateUser.bind(this)}
        isAgencyPage={this.props.isAgencyPage}
        isStationPage={this.props.isStationPage}
        isGeneralPage={this.props.isGeneralPage}
        isCustomerManagement={this.props.isCustomerManagement}
        isPersonnelPage={this.props.isPersonnelPage}
