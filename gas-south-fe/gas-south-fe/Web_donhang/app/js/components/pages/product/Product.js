...
import ExportEmptyCylindersWarehouse from "./ExportEmptyCylindersWarehouse";
...
import ExportDriverWarehousePopup from "../factory/ExportDriverWarehousePopup";
import ExportFactoryPopup from "../factory/ExportFactoryPopup";
import ExportFactoryPopup2 from "../factory/ExportFactoryPopup2";
...
import ImportFactoryStationPopup from "../factory/ImportFactoryStationPopup";
import ExportBackFactorySelection from "../factory/ImportBackFactorySelection";
...
import ExportCylinderInformationKhoXe from "../factory/ExportCylinderInformationKhoXe";
...
import ImportFactoryPopup2 from "../factory/ImportFactoryPopup2";
import ShellCylinder from "../factory/ShellCylinder";
import UpdateHistoryPopup from "./UpdateHistoryPopup";
import ExportFactoryNonSerialSelection from "../factory/ExportFactoryNonSerialSelection";
import ImportFactorySelection from "../factory/ImportFactorySelection";
import ExportFactorySelection from "../factory/ExportFactorySelection";
import ExportEmptyFactorySelectionFixer from "../factory/ExportEmptyFactorySelectionFixer";
import ExportEmptyFactorySelection from "../factory/ExportEmptyFactorySelection";
...
class Product extends React.Component {
  ...
  createDateTime = (date) => {
    ...
    const convertDateTime = `${dd < 10 ? "0" + dd : dd}-${
      mm < 10 ? "0" + mm : mm
    }-${yyyy}`;
  ...
  // handleOpenUpdateHistoryPopup = () => {
  //   const isOpen = this.state.openUpdateHistory;
  //   console.log('isOpen', isOpen)
  //   this.setState({ openUpdateHistory: !isOpen });
  // }
  ...
  async submitBuyCylinder(e) {
    ...
    // console.log('productResult', listCylinders)
    // return
    // Call api
    let cylinders = [];
    let cylinderImex = [];
    ...
    driver = data.shipper ? data.shipper : "Không xác định";
    ...
    if (dataUsers) {
      if (dataUsers.status === Constants.HTTP_SUCCESS_BODY) {
        ...
        for (let i = 0; i < dataUsers.data.length; i++) {
          listFactoryExports.push({
            value: dataUsers.data[i].id,
            // label: `${dataUsers.data[i].codetax ? 'GasSouth - ' : ''}${dataUsers.data[i].name}`,
            label: dataUsers.data[i].name,
            ...dataUsers.data[i],
          });
        }
  ...
  async handleClickSearch(page = this.state.activePage) {
    if (this.search_keyRef.value === "") {
      ...
      const valueSearch = this.search_keyRef.value.toUpperCase();
      if (valueSearch.length < 3) {
        showToast("Nhập tối thiểu 3 ký tự !");
      } else {
        await this.setState({
        ...
        await this.searchCylinder(valueSearch, page);
        await this.setState({
          loading: false,
        });
      }
      ...
      //     showToast("Nhập tối thiểu 3 ký tự chữ !");
      //   } else {
      //     await this.setState({
      //       searchKey: valueSearch,
      //       currentSkip: 1,
      ...
      //   }
      // } else {
      //   if (valueSearch.length !== 8) {
      //     showToast("Cần nhập đủ 8 ký tự số trong mã serial");
      //   } else {
      //     await this.setState({
      //       searchKey: valueSearch,
      //       currentSkip: 1,
  ...
  async searchCylinder(serial, page) {
    ...
    const dataProducts = await listPriceHistory(serial, page);
    ...
    if (dataProducts) {
      if (
        ...
        // console.log('data ne', dataProducts.data.data)
        this.setState({
  ...
  async getNotpassIdCylinders(products) {
    // console.log("Áaaaaa", products);
    this.setState({
      listIdNotpass: products,
    });
  }
    ...
    return (
      <td className="text-center table-actions">
        ...
        <a
          className="table-action hover-primary"
          data-toggle="modal"
          data-target="#update-history"
          onClick={() => {
            this.setState({
              // openUpdateHistory: true,
              product: store,
            });
          }}
        >
  ...
  render() {
    ...
    console.log(this.state.user_type, this.state.user_role);
    return (
      <div className="main-content">
            ...
            <div className="card-product-right-product">
              ...
              {this.state.user_type === "Fixer" ? (
                <button
                  className="btn btn-sm btn-danger kiemdinh"
                  data-toggle="modal"
                  data-target="#import-modal-selection"
                  style={{ backgroundColor: "#009347" }}
                >
                  Nhập vỏ không mã
                </button>
              ...
              this.state.user_role !== "Inspector" ? (
                <button
                  ...
                  className="btn btn-sm btn-danger kiemdinh"
                  data-toggle="modal"
                  data-target="#export-nonserial-modal-selection"
              ...
              {this.state.user_type === "Factory" &&
                ...
                this.state.user_role !== "Inspector" && (
                  ...
                  <button
                    className="btn btn-sm btn-success kiemdinh"
                    data-toggle="modal"
                    data-target="#export-back-modal-selection"
                    style={{ backgroundColor: "#009347" }}
                  >
                    Hồi lưu
                  </button>
              ...
              {this.state.user_type === "Factory" &&
                ...
                this.state.user_role !== "Inspector" && (
                  <button
                    className="btn btn-sm btn-danger kiemdinh"
                    data-toggle="modal"
                    data-target="#export-modal-selection"
                    style={{ backgroundColor: "#009347" }}
                  >
                    Xuất chai thành phẩm
                  </button>
                )}
              ...
              {this.state.user_type === "Fixer" ? ( // xuat vo cho tk sua chua
                <button
                  className="btn btn-sm btn-danger excell kiemdinh"
                  data-toggle="modal"
                  data-target="#export-empty-modal-selection-fixer"
              ...
              {this.state.user_type !== Constants.FIXER &&
                ...
                this.state.user_type !== Constant.PERSONNEL &&
                this.state.user_role !== "Deliver" &&
                this.state.user_role !== "Inspector" &&
                this.state.user_type !== "Region" && (
                  <button
                          ...
                          return (
                            <tr key={index}>
                              ...
                              <td scope="row" className="text-center">
                                ...
                                store.color !== "Không xác định"
                                  ? store.color
                                  : "Ghi trên vỏ chai"}
                              </td>
                              <td scope="row" className="text-center">
                                {store.valve !== "Không xác định"
                                  ? store.valve
                                  : "Ghi trên vỏ chai"}
                              </td>
                              <td scope="row" className="text-center">
                              ...
                              <th className="w-100px text-center">
                                {store.category &&
                                typeof store.category.name !== "undefined" &&
                              ...
                              <td>
                                ...
                                store.current ? (
                                  <p>
                                    <span> {store.current.name}</span>
                                    <br />
                                    <span>
                                      Địa chỉ: {store.current.address}
                                    </span>
                                    <br />
                                  </p>
                                ) : store.histories &&
                                  store.histories.length &&
                                  store.histories !== null &&
                                  store.histories[0].from &&
                                  store.histories[0].from.address &&
                                  store.histories[0].from.name &&
                                  store.histories[0].createdAt ? (
                                  <p>
                                    <span> {store.histories[0].from.name}</span>
                                    <br />
                                    <span>
                                      Địa chỉ: {store.histories[0].from.address}
                                    </span>
                                    <br />
                                    <span>
                                      {" "}
                              ...
                              <td>
                                ...
                                store.histories[1].createdAt ? (
                                  <p>
                                    <span> {store.histories[1].to.name}</span>
                                    <br />
                                    <span>
                                      Địa chỉ: {store.histories[1].to.address}
                                    </span>
                                    <br />
                                    <span>
                                      {" "}
                                  ...
                                  store.histories[0].isTruck ? (
                                  <p>
                                    <span> {store.histories[0].truckName}</span>
                                    <br />
                                    <span>
                                      Địa chỉ: {store.histories[0].truckAddress}
                                    </span>
                                    <br />
                                    <span>
                                      {" "}
        ...
        <ExportCylinder
          ...
          currentUser={this.state.user_cookies}
          ...
          getNotpassIdCylinders={(products) =>
            this.getNotpassIdCylinders(products)
          }
          getCylindersNotCreate={(products) =>
            this.getCylindersNotCreate(products)
          }
          cylinderNotPass={(value) => this.cylinderNotPass(value)}
        />
        <ExportCylinders
          product_parse={this.state.product_parse}
          ...
          listIdNotpass={this.state.listIdNotpass}
        />
        <ExportEmptyCylindersWarehouse
          product_parse={this.state.product_parse}
          typeExportCylinder={this.state.typeExportCylinder}
          parentRoot={this.state.parentRoot}
          listUsersPartner={this.state.listUsersPartner}
        ...
        <EditProductPopupContainer
          isViewMode={this.state.isViewProduct}
        ...
        {this.state.user_type === "Fixer" && (
          <ExportEmptyFactorySelectionFixer
            getListProducts={(products) => this.getListProducts(products)}
            getListCylinderDuplicate={(products) =>
              this.getListCylinderDuplicate(products)
            }
        ...
        {((this.state.user_type === "Factory" &&
          ...
          (this.state.user_type === "General" &&
            this.state.user_role !== "Deliver" &&
            this.state.user_role !== "Inspector")) && (
          <ExportFactoryNonSerialSelection
            getListProducts={(products) => this.getListProducts(products)}
            getListCylinderDuplicate={(products) =>
              this.getListCylinderDuplicate(products)
            }
        ...
        {((this.state.user_type === "Factory" &&
          ...
          <ExportCylinderInformation
            ...
            getCylindersNotCreate={(value) => this.getCylindersNotCreate(value)}
            cylinderNotPass={(value) => this.cylinderNotPass(value)}
          />
        )}
        ...
        {((this.state.user_type === "Factory" &&
          ...
          (this.state.user_type === "General" &&
            this.state.user_role !== "Deliver" &&
            this.state.user_role !== "Inspector")) && (
          <ExportCylinderInformationKhoXe
            getListProducts={(products) => this.getListProducts(products)}
            getSuccessIdCylinders={(products) =>
              this.getSuccessIdCylinders(products)
            }
        ...
        {
          // this.state.openUpdateHistory &&
          <UpdateHistoryPopup
            // handleOpenUpdateHistoryPopup={this.handleOpenUpdateHistoryPopup}
            product={this.state.product}
          />
        }
