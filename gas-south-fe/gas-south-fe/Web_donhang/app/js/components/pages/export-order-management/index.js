...
// import findOrderTank from "./../../../../api/findOrderTankAPI";
import openNotificationWithIcon from "../../../helpers/notification";
import PopupDetailExportOrder from "./popupDetailExportOrder";
import PopupInfoExportOrder from "./popupInfoExportOrder";
import showToast from "showToast"

class ExportOrderManagement extends Component {
  constructor(props) {
    super(props);
    this.state = {
      exportOrder: [],
      ...
      res.data._dataExportOrder.map((v, i) => {
        if (v.type === "X") {
          return data.push({
            ...
            code: v.code,
            id: v.exportId,
            // orderId: v.orderId.map((s) => {
            //   let symbol = (v.orderId.length > 1 )? ", " : "";
            //   let user = s + symbol;
            //   return user;
            // }),
            ordercode: v.ordercode,
            // userId: v.customerCode.map((s) => {
            //   let symbol = v.customerCode.length > 1 ? ", " : "";
            //   let orderCodee = s + symbol;
            //   return orderCodee;
            // }),
            userId: v.customerCode,
            // nameUser: v.customerName.map((s) => {
            //   let symbol = v.customerName.length > 1 ? ", " : "";
            //   let orderCodee = s + symbol;
            //   return orderCodee;
            // }),
  ...
  onClickEditCylinder = async (value) => {
    ...
    await callApi("POST", GETEXPORTORDERID, params, token).then((res) => {
      if (res) {
        console.log("GETEXPORTORDERID", res)
        if (res.status === 200) {
          let data = [];
          let data1 = [];
          let data2 = [];
          ...
          res.data._dataExportOrder.map((v) => {
            // v.exportOrderDetail.map((orderDetail, s) => {
            //   orderDetail.map((g) => {
            //     g.map((f) => {
            //       data.push({
            //         ...f,
            ...
            this.setState({
              ...
              () => {
                // console.log("orderIddddd", this.state.orderId);
              }
            );
          });
          data1.map((v) => {
          ...
          return (
            <div className="text-center statuss">
              ...
              <a
                ...
                data-toggle="modal"
                data-target="#detail-export-order"
                onClick={() => this.onClickEditCylinder(idd)}
              >
                <Icon type="edit" />
              </a>
      ...
      <div className="main-content" id="ExportOrderManagement">
        <div className="card">
          <div className="card-title d-flex justify-content-between">
            <h4>Quản lý lệnh xuất</h4>
            <div>
              <button className="btn btn-success mr-2">In lệnh xuất</button>
              <Link to="/create-export-order-management" className="p-0">
...
export default ExportOrderManagement;
