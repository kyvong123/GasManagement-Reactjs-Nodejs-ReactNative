...
// import getInfomationReturn from "getInfomationReturn";
...
import getInfomationReturn from "../../../../api/getInfomationReturn"
...
class ImportFactoryStationPopupFull extends React.Component {
  constructor(props) {
    super(props);
    this.form = null;
    this.state = {
  ...
  handleFileUpload(event) {
    ...
    fileReader.onload = async function (event) {
      ...
      if (array_id.length < 100) {
        let temp = await getInfomationReturn(array_id, "RETURN");
        console.log(temp);
        if (temp.status !== 200) {
        ...
        if (temp.data.err_msg) {
          ...
          errCyl_notEligibleForReturn = errCyl_notEligibleForReturn.concat(temp.data.errCyl_notEligibleForReturn);
          ...
          let tempIf = temp.data.success_cylinders.map((item) => item);
          success_cylinders = success_cylinders.concat(tempIf);
          let tempIf2 = temp.data.success_idCylinders.map((item, i) => item);
        ...
        for (let i = 0; i < 100; i += 10) {
          ...
          let temp = await getInfomationReturn(array, "RETURN");
          if (temp.status !== 200) {
            showToast("Xảy ra rỗi khi kết nối tới máy chủ");
            break;
          }
      ...
      resultSearch = {
        data: {
          ...
          errCyl_notEligibleForReturn: errCyl_notEligibleForReturn,
      ...
      if (resultSearch.status === 200) {
        ...
        if (
          ...
          errCyl_notEligibleForReturn.length ||
          ...
          if (errCyl_notCreated.length) {
            err5 = `Có ${errCyl_notCreated.length + errCyl_notInSystem.length} mã chưa khai báo.#`;
          }
          if (errCyl_notEligibleForReturn.length) {
            err7 = `Có ${errCyl_notEligibleForReturn.length} mã không hợp lệ.#`;
          }
          if (ab.length) {
            err6 = ab.toString();
          }
          errorShow = (err1 + err2 + err3 + err4 + err5 + err7 + err6).replace(regex, "\n").trim();
          that.setState({
            error: errorShow,
            listErr: errCyl_delivering
              ...
              .concat(errCyl_notEligibleForReturn),
            disableTab2: false,
            errCyl_delivering: errCyl_delivering,
            errCyl_current: errCyl_current,
            errCyl_inFixer: errCyl_inFixer,
  ...
  handleContinue = async () => {
    ...
    this.props.cylinderNotPassFull(
  ...
  render() {
    return (
      <div className="modal fade" id="export-cylinder-type-full" tabIndex="-1">
        {this.state.loading} && (<ReactCustomLoading isLoading={this.state.loading}>)
              ...
              >
                ...
                <footer className="card-footer text-center">
                  <button
                    ...
                    onClick={() => {
                      const modal = $("#export-cylinder-type-full");
                      // if (!this.state.errCyl_current.length && !this.state.errCyl_notCreated.length)
                      //   showToast('Phải tải lên tập tin danh sách mã bình !');
                      // else
                      modal.modal("hide");
                      this.handleContinue()
                    }}
                    type="submit"
                    data-toggle="modal"
                    // data-target={this.state.errCyl_current.length + this.state.errCyl_notCreated.length ? "#turn-back-driver-full" : ""}
                    data-target="#turn-back-driver-full"
                  >
                    Tiếp Tục
                  </button>
...
export default ImportFactoryStationPopupFull;
