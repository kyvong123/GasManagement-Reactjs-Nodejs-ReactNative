...
class ImportFactoryStationPopup extends React.Component {
  constructor(props) {
    super(props);
    this.form = null;
    this.state = {
      ...
      if (resultSearch.status === 200) {
        ...
        if (
          ...
          if (errCyl_current.length) {
            err2 = `Có ${errCyl_current.length} mã đã hồi lưu.#`;
          }
          if (errCyl_inFixer.length) {
            err3 = `Có ${errCyl_inFixer.length} mã ở nhà máy khác.#`;
          }
          if (errCyl_inFactory.length) {
            err4 = `Có ${errCyl_inFactory.length} mã ở trạm khác.#`;
          }
  ...
  render() {
    return (
      <div className="modal fade" id="import-cylinder-type-new" tabIndex="-1">
              ...
              >
                ...
                <footer className="card-footer text-center">
                  <button
                    ...
                    onClick={() => {
                      const modal = $("#import-cylinder-type-new");
                    ...
                    type="submit"
                    data-toggle="modal"
                    // data-target={this.state.errCyl_current.length + this.state.errCyl_notCreated.length ? "#turn-back-driver" : ""}
...
export default ImportFactoryStationPopup;
