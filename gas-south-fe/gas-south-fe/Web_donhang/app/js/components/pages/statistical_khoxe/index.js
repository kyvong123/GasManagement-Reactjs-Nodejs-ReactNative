...
import getStatisticByStationKhoXe from "../../../../api/getStatisticByStationKhoXe";
import getAggregateDashboard from "../../../../api/getAggregateDashboard";
import getStatisticAllRegionKhoXe from "../../../../api/getStatisticAllRegionKhoXe";
import getExportChart from "../../../../api/getExportChart";
import getDashboardFixer from "../../../../api/getDashboardFixer";
import getAggregateRepairFactoryKhoXe from "../../../../api/getAggregateRepairFactoryKhoXe";
...
function Statistical_V2() {
  ...
  async function getNewFixerDashboard(
    ...
    let typeCylinder = [];
    let resultDashboard = await getAggregateRepairFactoryKhoXe(
      target,
      startDate,
      endDate,
      statisticalType
            ...
            item.Export = item.Export.filter((element) => {
              if (
                ...
                element.exportEmptyCylinderObject.map((ele) => {
                  if (ele[0].newQuantity && ele[0].object.to == tramXuat)
                    tongBinhNew += ele[0].object.quantity;
                });
                element.numberNew = tongBinhNew;
                tongBinhNew = 0;
                element.exportEmptyCylinderObject.map((ele) => {
                  if (ele[0].oldQuantity && ele[0].object.to == tramXuat)
                    tongBinhOld += ele[0].object.quantity;
                });
  ...
  async function getAllDashboard(
    ...
    let result = await getStatisticAllRegionKhoXe(
      target,
      statisticalType,
      startDate,
      endDate
  ...
  async function getTableDashboard(
    ...
    let result = await getStatisticByStationKhoXe(
      target,
      statisticalType,
      startDate,
      endDate
          ...
          if (statisticType == 0) {
            ...
            if (statisticType == 3) {
              ...
              if (!khachHangHoiLuu || khachHangHoiLuu == "all") {
                ...
                objectSoLuongTramSearch.tongBinh = objectSoLuongTramSearch.tongBinh.toString();
