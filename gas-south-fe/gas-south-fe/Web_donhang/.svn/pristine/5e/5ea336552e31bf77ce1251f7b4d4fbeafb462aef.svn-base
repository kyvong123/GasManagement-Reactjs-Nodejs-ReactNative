...
class ExportTypeCylinderPopup extends React.Component {
  ...
  handleFileUpload(event) {
    ...
    fileReader.onload = async function (event) {
      ...
      // }
      let resultSearch = await getInformationFromCylinders(cylinders_list, Constant.GIVE_BACK_TYPE);
  ...
  render() {
    return (
      <div className="modal fade" id="export-type-cylinder-popup" tabIndex="-1">
              ...
              <Form ref={c => {
                ...
                <footer className="card-footer text-center">
                  <button className="btn btn-primary"
                    ...
                    onClick={() => {
                      const modal = $("#export-type-cylinder-popup");
                      modal.modal('hide');
                    }} type="submit" data-toggle="modal"
                    data-target="#export-driver-type-cylinder">Tiếp Tục
                                    </button>
...
export default ExportTypeCylinderPopup;
