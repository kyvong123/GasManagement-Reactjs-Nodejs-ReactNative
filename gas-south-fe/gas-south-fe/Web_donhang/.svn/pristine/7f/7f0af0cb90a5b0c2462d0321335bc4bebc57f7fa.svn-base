...
class TurnBackDriverFactoryPopupFull extends React.Component {
    ...
    async addHistory(actionType,driver,idDriver,license_plate,cylinder,reason) {
        ...
        if (this.props.listIdSuccess.length !== 0) {
            ...
            if (result) {
                if (result.status === Constant.HTTP_SUCCESS_CREATED || result.status === Constant.HTTP_SUCCESS_BODY && !result.data.hasOwnProperty("err_msg")) {
