...
import statisticsApi from "../../api/statisticwarehouse";
...
export default function StatisticManagerWarehouse() {
  ...
  const [exportStationsVehicle, setExportStationsVehicle] = useState([]);
  const [exportStations, setExportStations] = useState([]);
  const [fromValue, setFromValue] = useState("");
  const [isVisibleFrom, setIsVisibleFrom] = useState(false);
  const [isLoadRegion, setIsLoadRegion] = useState(false);
  ...
  const chooseStatisticTime = (time) => {
    ...
    switch (time) {
      ...
      case "YESTERDAY":
        ...
        let newPreviousEndDate = new Date().setHours(23, 59, 59, 999);
  ...
  const getAllStations = async () => {
    ...
    await statisticsApi.getWarehouse("all", token).then((data) => {
      if (data && !data.data.error) {
        const response = handleDataAllStations(data.data.data);
        setExportStationsVehicle(response);
      }
    });
      ...
      if (statisticValue == "All") {
          ...
          cylinderTypes.map((type) => {
            ...
            if (!children.has(type)) {
              ...
              if (finder?.exportEmptyCylinderObject) {
                const numberOld = finder.exportEmptyCylinderObject.reduce(
                  (accumulator, current) => {
                    if (
                ...
                const numberNew = finder.exportEmptyCylinderObject.reduce(
                  (accumulator, current) => {
                    if (
        ...
        cylinderTypes?.map((type, index) => {
          ...
          if (item?.exportEmptyCylinderObject) {
            const numberNew = item.exportEmptyCylinderObject.reduce(
              (accumulator, current) => {
                if (
                  current.hasOwnProperty("newQuantity") &&
                  Object.keys(current).length > 0
        ...
        cylinderTypes?.map((type, index) => {
          const item = _data[statistic].find((i) => i.code.includes(type));
          if (item?.exportEmptyCylinderObject) {
            const numberOld = item.exportEmptyCylinderObject.reduce(
              (accumulator, current) => {
                if (
                  current.hasOwnProperty("oldQuantity") &&
                  Object.keys(current).length > 0
        ...
        if (
          ...
          await statisticsApi
            ...
            .then((data) => {
              ...
              if (data && data.data.success) {
                const res = handleResponseAllDataRegion(
                  data.data.data,
                  listStation,
                  dataColumn
                );
    ...
    <>
      {listBranchs.length > 0 ? (
        ...
        >
          ...
          {table == "exportCylinder" && (
            <View style={styles.optionSection}>
              <Text style={styles.txtOption}>{translate("TO_CUSTOMER")}</Text>
              <DropDownPicker
                style={styles.dropdownStyle}
                items={customers}
          ...
          {table == "exportEmptyCylinder" && (
            <View style={styles.optionSection}>
              <Text style={styles.txtOption}>{translate("TO")}</Text>
              <DropDownPicker
                style={styles.dropdownStyle}
                items={customers}
                open={isVisibleToCustomer}
          ...
          {((table == "exportEmptyCylinder" && customerValue == "Station") ||
            (table == "importEmptyCylinder" && fromValue == "Station") ||
            (table == "exportCylinder" && customerValue == "Station")) && (
            <View style={styles.optionSection}>
              <Text style={styles.txtOption}>{translate("VEHICLE")}</Text>
              <DropDownPicker
                searchable={true}
                    ...
                    table?.map((statistic) => {
                      return (
                        <View style={styles.tableStyle}>
                          ...
                          customerValue != "All" &&
                          statisticValue ==
                            "exportEmptyCylinder" ? null : renderTotalX(
                              branchValue,
                              statisticValue
                            ) ? (
                            <View style={styles.rowStyle}>
