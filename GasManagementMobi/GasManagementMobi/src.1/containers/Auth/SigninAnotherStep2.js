...
class SigninAnotherStep2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      password:'',
...
SigninAnotherStep2.propTypes = propTypes;
...
const mapStateToProps = state => ({
  ...
  ternants: getTernant(state.get('AuthReducer').get('anotherTernants').toJS()),
  loading: state.get('AuthReducer').get('loading'),
  error: state.get('AuthReducer').get('errorStep2')
});

export default connect(mapStateToProps,{loginUser})(withNetInfo(SigninAnotherStep2));
