...
import {
    addCylinder,
    fetchCylinder,
    updateCylinders, deleteCylinders, typeForPartner,
...
//let lastCylinders = []
class DriverScanResults extends Component {
    constructor(props) {
        super(props);
        ...
        return (
            <View style={{ flexDirection: "column" }}>
                <View style={styles.title}>
                    <View>
                        <TextInput
                            style={{ height: 40, width: Platform.OS === 'ios' ? 320 : 260, marginRight: 10 }}
    ...
    renderInfo = () => {
        ...
        // if (this.state.cnt === 0) {
        //     if (cyclinder.hasDuplicate || cyclinder.isDuplicate) {
        //         this.props.getDuplicateCylinder(this.props.userInfor.isChildOf, cyclinder.id, cyclinderAction);
        //         this.ShowAlert(cyclinder.id,cyclinderAction,this.props.userInfor.isChildOf)
        //     }
        //     this.setState({cnt: this.state.cnt+1})
        // }

        //console.log('cyclinderAction', cyclinderAction);
        return (
            <View>
                <List>
                    <Item
                    ...
                    {/*<Item extra={<Text style={styles.itemStyle}>{cyclinder.serial}</Text>}><Text*/}
                    {/*    style={styles.itemStyle}>Số serial</Text></Item>*/}
                    <Item
                        extra={
                            //<Text style={styles.itemStyle}>{cyclinder.checkedDate}</Text>
              ...
              extra={
                  <Text style={styles.itemStyle}>
                      {cyclinder.currentSalePrice !== 0
                          ? cyclinder.currentSalePrice
                              .toString()
                              .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")
                          : cyclinder.currentImportPrice
                              .toString()
                              .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")}
                  </Text>
              }
            >
              <Text style={styles.itemStyle}>{translate('PRICE')} (VNĐ) </Text>
            </Item> */}
                    ...
                    {/* <View style={{flexDirection: 'row'}}>
                                <Text style={{flex:2/3,color: COLOR.BLUE,fontSize: 15}}>{"  "+ " " + translate('Current_weight')} (kg)</Text>
                                <TextInput style={{flex:1/3,textAlign:"right",borderColor: 'gray',borderWidth: 1}}
                                    defaultValue = {cyclinder.weight.toString()}
                                    // label = {cyclinder.weight}
                                    valve = {cyclinder.weight}
                                    onChangeText={(weight) => {this.setState({weight})}}
                                    // error={this.state.errorWeight}

                                    // {console.log('this.setState.weight',this.setState.weight)}
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {cyclinder.placeStatus !== "IN_FACTORY" ? (
                        <View style={styles.btnReport}>
                            ...
                            <TouchableOpacity style={styles.btnStamp} onPress={Actions.ManufactureInfo}>
                                <Text style={styles.txtBtnStamp}>{translate('STAMPS_BRANDS')}</Text>
                            </TouchableOpacity>
                        </View>
                    ) : null}
                </View>
    ...
    render() {
        ...
        // }
        //neu checkdata false thi hien render
        if (!this.state.checkdata) {
            return this.renderHeader();


        }
        // if (!isEmpty(cyclindererr)) {
        //   console.log("khong thay",cyclindererr)
        //   return this.renderHeader();
        // }
        ...
        return (
            <ScrollView style={styles.container}>
                ...
                {/* {cyclinder.weight > 15 ? (<Modal isVisible={this.state.openModal}>
                   <View style={{width : '100%',backgroundColor : '#FFF'}}>
                        <View style={{width : '100%', justifyContent : 'center', alignItems : 'center', flexDirection : 'row'}}>
                            ...
                            <TextField
                                ...
                                keyboardType='numeric'
                                onChangeText={(e) => {this.setState({weight : e, errorWeight : false})}}
                                error={this.state.errorWeight}
                            />
                            ...
                            <Button                             
                                type="primary"
                                onPress={ async () => { 
                                    console.log(cyclinder.serial);
                                    
                                    if(!this.state.weight) {
                                        this.setState({errorWeight : translate('WEIGHT_IS_REQUIRED')})
                                    }else {
                                        let check = await addLocalCyclinder.addItem(cyclinder.id,cyclinder.serial,this.state.weight);
                                        let data = await addLocalCyclinder.getItem();
                                ...
                                style={[styles.btnLogin, {marginVertical : 20}]}
                                >
                                    {translate('FINISH')}
                                </Button>
