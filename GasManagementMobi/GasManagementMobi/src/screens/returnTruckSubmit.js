...
const returnTruckSubmit = (props) => {
    ...
    const validate = values => {
        ...
        if (!values.driverName && typeName === DRIVER_TYPE.EXTERNAL_DRIVER) {
    ...
    const printDriverName = ({ driverId, driverName }) => {
        let _driverName = driverName
        if (typeName === DRIVER_TYPE.INTERNAL_DRIVER) {
            const { name } = driverList.find(driver => driver.id === driverId)
            _driverName = name
        }
        return _driverName ?? ''
    }
    ...
    const onSubmit = async form => {
        let driverNameTemp = form.driverName
        if (typeName === DRIVER_TYPE.INTERNAL_DRIVER) {
            const { name } = driverList.find(driver => driver.id === form.driverId)
            driverNameTemp = name
        }
        console.log("form.driverName", driverNameTemp);
                            ...
                            <Center style={tw('mt-7')}>
                                <VStack width="90%" space={4}>
                                        ...
                                        <View>
                                            <Radio.Group
                                                ...
                                                onChange={value => {
                                                    ...
                                                    setFieldValue("licensePlate", value === DRIVER_TYPE.EXTERNAL_DRIVER ? value.licensePlate : userInfo.license_plate);
                                                    setTypeName(value);
                                                }}>
                                                ...
                                                <Radio value={DRIVER_TYPE.EXTERNAL_DRIVER} my={1}>
                                                    <FormControl.Label>Xe ngoài:</FormControl.Label>
                                                </Radio>
                                            </Radio.Group>
                                    ...
                                    <FormControl isRequired isInvalid={'driverName' in errors}>
                                        <FormControl.Label>Tên tài xế: </FormControl.Label>
                                        {typeName === DRIVER_TYPE.INTERNAL_DRIVER ?
                                            <View>
                                                <Select selectedValue={values.driverId} minWidth="200" placeholder="Chọn tên" _selectedItem={{
                                                    bg: "teal.600",
                                                    endIcon: <CheckIcon size="5" />
                                                }} mt={1}
                                                    onValueChange={handleChange('driverId')}>
                                                    ...
                                                    try {
                                                        ...
                                                        await BluetoothEscposPrinter.printColumn(
                                                            ...
                                                            [
                                                                'Dia Chi',
                                                                ': ',
                                                                values.address ? cleanAccents(values.address) : '',
                                                            ],
                                                            { fonttype: 1 },
                                                        );
                                                        await BluetoothEscposPrinter.printColumn(
...
export default returnTruckSubmit;