...
export default ItemOrder = ({ item, index }) => {
    ...
    const handleDelivery = () => {
        dispatch(truckStoreCurrentOrder({
            orderDetail: orderDetail,
            orderOverview: item
        }));
        dispatch(changeTypeDeliveryTruckAction(
            item.orderType === 'KHONG' ? SHIPPING_TYPE.GIAO_HANG : SHIPPING_TYPE.GIAO_VO
        ))
        saver.setTypeCamera(false);
        Actions.push('truckDeliveryScan');
    }
                ...
                <View>
                    ...
                    <View>
                        ...
                        {deliveryHistorys != null && deliveryHistorys != undefined ?
                            <View>
                                {(deliveryHistorys ? deliveryHistorys : []).map(item => <TruckDeliveryHistoryOrderItem {...item} />)}
                            </View> :
